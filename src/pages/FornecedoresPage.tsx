...
import { Building2, Plus, FileText, FileSpreadsheet, Pencil, Trash2, MessageCircle } from "lucide-react";
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {session.permissoes.gerarExcel && (
          <Button variant="outline" onClick={handleExcel} className="gap-2 border-accent/50 text-accent hover:bg-accent/10 font-orbitron text-sm h-14 rounded-xl neon-glow-green uppercase">
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="glass-card border-primary/30 max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-orbitron text-primary neon-text text-sm uppercase">{editingId ? "EDITAR FORNECEDOR" : "NOVO FORNECEDOR"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            <Input placeholder="NOME DA EMPRESA / RAZÃO SOCIAL" value={razaoSocial} onChange={e => setRazaoSocial(e.target.value)} className="text-center bg-input border-border/50 focus:border-primary h-14 font-orbitron text-xs uppercase" />
            <Input placeholder="CNPJ / CPF" value={cnpjCpf} onChange={e => setCnpjCpf(e.target.value)} className="text-center bg-input border-border/50 focus:border-primary h-14 uppercase" />
            <OptionGroup label="TIPO DE FORNECEDOR" value={tipo} onChange={v => setTipo(v as TipoFornecedor)} colorClass="bg-primary text-primary-foreground" glowClass="neon-glow-primary" options={TIPOS} />
            <Input placeholder="TELEFONE DE CONTATO" value={telefone} onChange={e => setTelefone(e.target.value)} className="text-center bg-input border-border/50 focus:border-primary h-14 uppercase" />
            <Input placeholder="CIDADE / ESTADO" value={cidadeEstado} onChange={e => setCidadeEstado(e.target.value)} className="text-center bg-input border-border/50 focus:border-primary h-14 uppercase" />
            <Textarea placeholder="OBSERVAÇÕES" value={observacoes} onChange={e => setObservacoes(e.target.value)} className="bg-input border-border/50 focus:border-primary uppercase min-h-[80px]" />
                ...
                <TableRow key={f.id} className="border-border/20">
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      {f.telefone && (
                        <a href={formatWhatsApp(f.telefone)} target="_blank" rel="noopener noreferrer"
                          className="h-8 w-8 rounded-full bg-accent/20 flex items-center justify-center text-accent hover:bg-accent/30 transition-all neon-glow-green">
                          <MessageCircle className="h-4 w-4" />
                        </a>
                      )}
                      <button onClick={() => openEdit(f)} className="text-primary hover:text-primary/80"><Pencil className="h-4 w-4" /></button>
                      <button onClick={() => handleDelete(f.id)} className="text-destructive hover:text-destructive/80"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
