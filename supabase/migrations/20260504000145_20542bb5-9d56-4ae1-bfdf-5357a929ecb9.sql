...
ADD COLUMN IF NOT EXISTS pode_os BOOLEAN NOT NULL DEFAULT false;

UPDATE public.profiles
SET pode_os = COALESCE(pode_os, false) OR COALESCE(pode_patio, false) OR COALESCE(pode_rodizio, false)
WHERE pode_os = false;
...
BEGIN
  SELECT id, nome, login, nivel, ativo,
         ...
         pode_fornecedores, pode_expedicao, pode_os, pode_pdf, pode_excel, pode_lavacao,
         pode_bloqueados
  INTO v_user
  FROM public.profiles
